...
bundled into `public/index.js` by esbuild. Upstream serves that URL `no-cache`
and updates it in place, so loading it live would hand a third party a
standing remote-code channel into an origin that holds decrypted group data.
