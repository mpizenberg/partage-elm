...
export function signChallenge(challenge, timestamp, difficulty, secret = TEST_SECRET) {
  return createHmac('sha256', secret)
    .update(`${challenge}:${timestamp}:${difficulty}`)
    .digest('hex');
}

/** Fetch a challenge from the app, solve it, and return the pow_* body fields. */
export async function solvedPow(app) {
