...
- Increment 5 done: `packages/pb_server/` and the pocketbase.yml workflow deleted; relay Dockerfile (one container, one volume); DEPLOY.md rewritten (Workers / container, same-origin builds via empty `SERVER_URL`); spec Appendix C + §11 rewritten for the relay; README updated; CI bumped to Node 24 and runs both relay suites. Deviations: the **real-browser two-tab flow remains untested** — check it before deploying. (The Docker image was later built and smoke-tested: container serves the frontend and answers `/api/pow/challenge`.)
- Follow-up: CI now builds and pushes the relay image to ghcr.io on every push to `main` (`.github/workflows/relay-image.yml`); DEPLOY.md gained a Dokku section deploying that image via `git:from-image`.
- Bug fix found in first real deploy: invite links (`/join/<id>`) 404ed — neither adapter had an SPA fallback. Node adapter now serves `index.html` for unknown non-`/api` paths; Worker assets use `not_found_handling: single-page-application`.
- Increment 4 done: Cloudflare adapter (`src/worker.js` Worker + `GroupDo` Durable Object over `src/storage-do.js`, hibernating WebSockets, static assets from `dist/`). Shared conformance suite (`test/conformance.js`) runs on both adapters: 30 node tests + 18 workers tests green; `wrangler deploy --dry-run` validates. Includes a protocol change: PoW challenges are now bound to the groupId (see Decisions).
...
- **Relay image publishes on every push to `main`** (plus manual dispatch), tagged `latest` and commit sha. Alternative: dispatch-only like the old pocketbase workflow. Chosen because the CI-built image is the primary deploy artifact for self-hosting; sha tags exist because Dokku's `git:from-image` never redeploys a tag it has already seen.
- **Dokku deploys from the registry image, not `git push`.** A git-push deploy would need a multi-stage Dockerfile building the frontend (Elm toolchain) inside Docker on the VPS. Rejected for now to keep the image and the build simple; revisit if registry auth becomes a nuisance.
...
- **Same-origin deploys build with an empty `SERVER_URL`** — the esbuild define now uses `${SERVER_URL-default}` (empty string is a real value, only *unset* falls back to `http://127.0.0.1:8090`), and the client falls back to `location.origin` when the injected value is empty. Alternative rejected: baking the production URL into the bundle, which would tie one build to one host.
- **PoW challenges are bound to the groupId** (`GET /api/pow/challenge?groupId=…`, HMAC covers `challenge:groupId:timestamp:difficulty`). Replay protection previously relied on a globally-unique stored challenge, which cannot work on Cloudflare where each group's storage is an isolated Durable Object. Binding makes replay useless statelessly (a solved challenge only fits one group, creatable once), so the `challenge_used` conflict path and the unique constraint became dead code and were deleted. Alternative rejected: a global "challenge registry" DO — extra state and a cross-DO call for no benefit.
- **Conformance suite is shared by injection** — `test/conformance.js` takes `{describe, it, makeApp}` and runs under both node:test (in-process Hono) and vitest-pool-workers (real workerd via `SELF`). vitest-pool-workers ≥0.18/vitest 4 dropped per-test isolated storage, so the suite allocates unique group ids per test instead of relying on isolation.
- **The Worker answers `/api/pow/challenge` and group-creation routing itself** (stateless / needs the body's groupId to pick the DO); everything else under `/api/groups/:id/` goes to that group's DO, and non-API paths fall through to static assets. The hosted instance is same-origin, so the Worker-level routes skip the CORS middleware that lives in the portable core.
