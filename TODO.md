...
Phases 1–10 are complete. Remaining: translations.
...
- **10.3** Conditional gzip compression: `src/Compression.elm` + JS tasks in `public/index.js` combine compression and AES-256-GCM encryption in a single JS round-trip via `CompressionStream`/`DecompressionStream` Web APIs. Compression is applied only when gzip reduces the payload by at least 30%. A `compressed` boolean field on the PocketBase events record signals whether decompression is needed on pull. Benchmarks on realistic payloads: single small events (<200B) are not compressed (gzip header overhead), single expenses (500–900B) save 30–40%, batched payloads benefit most — group creation (4 events, 886B): 51%, typical sync (3 expenses, 2KB): 68%, large sync (20 expenses, 16KB): 82%.
