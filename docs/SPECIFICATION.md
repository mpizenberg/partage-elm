...
| **Payment methods** | IBAN, Wero, Lydia, Revolut, Wise, PayPal, Venmo, Cash App, Bitcoin (BTC address), Cardano (ADA address) |
...
- Some payment methods generate clickable payment links (Lydia, Revolut, Wise, PayPal, Venmo, Cash App, Bitcoin).
...
```
POST /api/groups/:id/compact   { uptoSeq, expectedCount, records: [...] }
```

The caller takes `uptoSeq` as the highest seq of its own fresh pull and
`expectedCount` as how many records that pull held at or below it. The
server, in one transaction: verifies `uptoSeq` ≤ current max seq **and that
exactly `expectedCount` records remain in the delete range**, deletes
records with `seq ≤ uptoSeq`, and appends the consolidation records at
fresh, strictly higher seqs (AUTOINCREMENT never reuses). Racing compactors
serialize: the loser's snapshot is stale — its range covers deleted rows or
a count that no longer matches — and is rejected with 409 (re-pull and
retry or give up). Concurrent pushes sit above `uptoSeq` and never trip the
count check. The uploaded records must contain every
...
| `POST` | `/api/groups/{id}/compact` | bearer | Consolidate the log ([14.9](#149-log-consolidation-compaction)): `{uptoSeq, expectedCount, records}` transactionally deletes records with `seq ≤ uptoSeq` and appends `records` at fresh higher seqs, provided the delete range still holds exactly `expectedCount` records. Returns `{maxSeq}`; `409` when the caller's snapshot went stale (lost race — re-pull and retry or give up); `507`/`429` accounted net, growth only. |
...
### C.7 Operator Observability (Self-Host)

An optional read-only surface for the **operator** — not members — to monitor a deployment: capacity, growth, relay-observable abuse, per-group hot-lists, and a pseudonymous users estimate. It is enabled only when `ADMIN_SECRET` is set (absent `404` otherwise), and is **self-host only** — Cloudflare parity is deferred, because one Durable Object per group gives no global index for fleet rollups. Configuration and hardening live in [DEPLOY.md](DEPLOY.md#operator-dashboard-self-host).

- `GET /api/admin/summary` — a JSON aggregate, bearer-authenticated with `ADMIN_SECRET`, a **separate** operator credential compared in constant time exactly like the group verifier ([C.5](#c5-authentication)). It is strictly higher privilege than any group secret because it reads across all groups, so it is its own key, never a reused `POW_SECRET` or group bearer. Sections: `now` (live fleet levels), `history` (a day-bucketed series, `?days=` windowed, clamped to one year), `hotlists` (top-N opaque `groupId`s by bytes / records / age / distinct actors), `flags` (threshold breaches evaluated at read time), and `cost`.
- `GET /admin` — a self-contained page that renders that JSON. The secret is entered by the operator and held only in the browser tab session; the page ships outside the member bundle.

**Zero-knowledge boundary.** The surface exposes only what the relay can already see ([11.6](#116-what-the-server-can-see)) — `groupId`, `actorId`, sizes, timestamps — in aggregate. It never decrypts or returns group content, names, members, or amounts. It also never surfaces the per-device tamper and suspicion signals ([11.7](#117-compromised-member-threat-model)): those are deliberately local and would break their zero-tell contract if collected centrally. Operator security (relay-observable abuse) and member security (local compromise detection) are separate datasets that never cross.

**"Users" is always an estimate.** With no accounts, the only proxy for a user is a distinct `actorId`, which over-counts (one human, several devices) and can never dedupe humans. Both figures are labelled estimates: a live active-window count (1 / 7 / 30-day) and a cumulative count across retained history — the latter under-counts over time, since retention purges idle groups' events ([14.8](#148-relay-retention--storage-limits)), which is why the daily snapshot, not a live recompute, preserves the historical cumulative.

**Metadata trade-off.** Hot-lists widen the surface beyond aggregates — they reveal which opaque group is largest, oldest, or busiest (still no content), a finer fingerprint if the secret leaks. This is an accepted cost of the drill-down. Because the surface can sit on the public origin, the secret is the primary control: a strong random `ADMIN_SECRET`, compared in constant time, guarded by a strict **per-IP lockout on failed authentication** (five failures → a 15-minute block, keyed off the proxy's `X-Forwarded-For` so the operator cannot be framed into a lockout). Restricting the paths to an operator-only network is optional defense-in-depth on top. The data comes from a day-bucketed `daily` table — flow counters (rejections, groups created, bytes served, compactions, purges) plus a daily level snapshot (totals, size percentiles, actor estimates) written by the existing retention sweep — because the live tables hold only the present.

**Cost.** A current monthly run-rate from real figures, reusing the member estimate's rates ([18.2](#182-cost-estimation)): storage and compute from stored bytes, network from a trailing-30-day egress sum, base flat. Unlike the member view it closes the network line from a real `bytes_served` counter rather than a guess.
