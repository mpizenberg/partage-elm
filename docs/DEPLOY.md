...
`STATIC_DIR` is a complete, immutable deployment unit, not a directory to populate or rebuild while the relay runs. Its `index.html`, `sw.js`, `robots.txt`, and `sitemap.xml` are required at startup; the relay stops immediately when the build is incomplete. Deploy a replacement build by restarting the relay with the new directory/image.
