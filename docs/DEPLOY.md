...
`STATIC_DIR` is a complete, immutable deployment unit, not a directory to populate or rebuild while the relay runs. Its app shell (`app.html`), page manifest (`pages.json`) with every page it declares, service worker, and crawler-discovery files are required at startup; the relay stops immediately when the build is incomplete. Deploy a replacement build by restarting the relay with the new directory/image.
...
If you would rather the metadata surface be unreachable from the internet as well, that is optional defense-in-depth: restrict `/admin` and `/api/admin/*` with an nginx `allow`/`deny` block on those paths, or reach them only over an SSH tunnel or a private overlay (e.g. Tailscale).
