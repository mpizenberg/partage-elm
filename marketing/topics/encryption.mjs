export default {
  ...
  en: {
    ...
    tagline: "What “end-to-end encrypted” means here — precisely, including what the server can still see.",
    open: "Open Partage",
    home: "Home",
...
<p>Everything a group contains — expenses, transfers, members, categories, every edit in the history — is encrypted on your device before it syncs. What leaves the device is ciphertext, and what the server stores and forwards is ciphertext.</p>
<p>The key travels only when you invite someone, and only inside the invite link, as described below. It never reaches the server.</p>`,
      },
      {
        ...
        body: `<p>A privacy claim is only worth something when it names what the operator <em>could</em> observe. The sync relay sees:</p>
<ul>
...
<li>aggregate counts for these public pages: landings per day, the top referring sites, and which page was served — one counter per page from a fixed list, never the address in your URL bar. There is no analytics script, no cookie, and no per-visitor identifier.</li>
</ul>
<p>It cannot see amounts, titles, categories, currencies, member names, balances, or who owes whom. Not “does not look at” — cannot. The relay never receives the key: a device proves its access with a hash of the key, and the server stores only a hash of that hash. A full copy of the server’s database reveals no expense to anyone.</p>
<p>The relay also keeps nothing forever: a group untouched by every member for twelve months is deleted from the server, while each member’s device keeps its own full copy.</p>`,
      },
      {
        id: "invitations",
...
<p>The flip side is worth saying plainly: anyone who has the link has the key. Share it the way you would share a house key — directly with the people you mean to let in.</p>`,
      },
      {
        id: "honest-limits",
        title: "The honest limits",
        body: `<p>Three things this page will not oversell:</p>
<ul>
...
<li>Partage is a web app, so the code that encrypts is delivered by the server. An operator serving malicious code could read your data — the tradeoff every encrypted web app makes. Open source and an installable app soften this; they do not remove it.</li>
<li>Encryption hides content, not shape. Sizes and timing of encrypted records are visible to the relay, as listed above.</li>
  ...
  fr: {
    ...
    description:
      "Ce que « chiffré de bout en bout » veut dire dans Partage : des clés AES-256 qui ne quittent jamais vos appareils, ce que le relais voit — et ne peut pas voir — et les limites honnêtes.",
    heading: "Comment Partage chiffre vos données",
    tagline: "Ce que « chiffré de bout en bout » veut dire ici — précisément, y compris ce que le serveur voit encore.",
    open: "Ouvrir Partage",
    home: "Accueil",
...
<p>Tout le contenu d’un groupe — dépenses, remboursements, membres, catégories, chaque modification de l’historique — est chiffré sur votre appareil avant la synchronisation. Ce qui quitte l’appareil est du chiffré, et ce que le serveur stocke et retransmet est du chiffré.</p>
<p>La clé ne voyage que lorsque vous invitez quelqu’un, et uniquement dans le lien d’invitation, comme décrit plus bas. Elle n’atteint jamais le serveur.</p>`,
      },
      {
        ...
        body: `<p>Une promesse de confidentialité ne vaut que si elle nomme ce que l’opérateur <em>pourrait</em> observer. Le relais de synchronisation voit :</p>
<ul>
...
<li>des compteurs agrégés pour ces pages publiques : visites par jour, principaux sites référents, et quelle page a été servie — un compteur par page d’une liste fixe, jamais l’adresse de votre barre d’URL. Pas de script d’analyse, pas de cookie, pas d’identifiant de visiteur.</li>
</ul>
<p>Il ne peut voir ni montants, ni intitulés, ni catégories, ni devises, ni noms de membres, ni soldes, ni qui doit quoi à qui. Pas « ne regarde pas » — ne peut pas. Le relais ne reçoit jamais la clé : un appareil prouve son accès avec un condensat de la clé, et le serveur ne stocke qu’un condensat de ce condensat. Une copie complète de la base du serveur ne révèle aucune dépense à personne.</p>
...
<p>Le revers mérite d’être dit clairement : qui a le lien a la clé. Partagez-le comme vous partageriez la clé de chez vous — directement aux personnes que vous voulez faire entrer.</p>`,
      },
      {
        id: "limites-honnetes",
        title: "Les limites honnêtes",
        body: `<p>Trois choses que cette page ne survendra pas :</p>
<ul>
...
<li>Partage est une application web : le code qui chiffre est livré par le serveur. Un opérateur servant du code malveillant pourrait lire vos données — le compromis de toute application web chiffrée. Le code ouvert et l’installation en application l’atténuent sans le supprimer.</li>
