...
const transformNotification = readFileSync(
  "public/sw-transform-notification.js",
  "utf-8",
);

if (process.env.DEV_SERVICE_WORKER === "1") {
  buildDevelopmentServiceWorker();
} else {
  buildProductionServiceWorker();
}

function buildDevelopmentServiceWorker() {
  const generated = generateSW({
    cacheName: "partage-development",
    precacheUrls: [],
  ...
  // Taking control removes a previously installed production worker before it
  // can keep serving an Elm bundle tied to an obsolete elm-watch process.
  const takeControl = `
...
`;
  writeFileSync("dist/sw.js", generated + takeControl);
}

function buildProductionServiceWorker() {
  ...
  // ordinary update instead of stranding them on a CSP that predates it.
  const cacheName =
    "partage-" + digest.digest("hex").slice(0, 16) + "-__CONFIG_DIGEST__";
